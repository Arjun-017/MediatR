...
/// </summary>
public interface IMediater : ISender, IPublisher { }
...
public interface ISender {
    ...
    Task SendAsync(IRequest query, CancellationToken token = default);
    /// <summary>
    /// Sends an event
    /// </summary>
    /// <param name="query">The event that extends IQuery<typeparamref name="T"/></param>
    /// <param name="token">CancellationToken token</param>
    Task<T> SendAsync<T>(IRequest<T> query, CancellationToken token = default);
}
